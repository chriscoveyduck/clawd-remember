...
describe("MemoryManager", () => {
    it("captures extracted facts and supports recall", async () => {
        const manager = new MemoryManager(new InMemoryStorageProvider(), new MockEmbedder(), new MockExtractor([
    ...
    it("adds and deletes facts manually", async () => {
        const manager = new MemoryManager(new InMemoryStorageProvider(), new MockEmbedder(), new MockExtractor([]), { userId: "user-1" });
        await manager.init();
